...
public class AccountController {
	...
	private void delete() {
		...
		if(itemList == null || itemList.size() ==0) {  //itemlist데이터베이스가 비어 있으면 수정할 내역이 없다고 반환
	...
	private void view() {
		...
		do {
			Criteria cri = new Criteria(page,2); //1페이지당 2개씩 보여주기 위함
			cri.setSearch(date);
	...
	private void update() {
		...
		try {
			...
			if(accountService.updateItem(item)) {
				...
				System.out.println("내역 수정이 완료됐습니다.");
			}
	...
	private Item inputItem() throws ParseException { //전체 내역을 수정하기 위해 쓴 함수
		//수입/지출 선택
	...
	private void insert() {
		...
		try {
			Item item = inputItem();	//전체 내역 수정한 것을 다시 item에 넣는 작업
